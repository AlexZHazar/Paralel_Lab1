...
if not result_files:
    ...
    for c_file in result_files:
        ...
        # Позиция максимальной абсолютной ошибки
        idx = np.unravel_index(np.argmax(abs_diff), abs_diff.shape)

        # Вывод подробностей со смайликами
        ...
        rtol = 1e-4
        atol = 1.0
        if np.allclose(C_ref, C_prog, rtol=rtol, atol=atol):
            print(f"  ✅ Верификация ПРОЙДЕНА (rtol={rtol}, atol={atol})")
        else:
        ...
        rows.append({
            ...
            "position": str(idx),
            "passed": "✅" if np.allclose(C_ref, C_prog, rtol=rtol, atol=atol) else "❌"
        })
